...
def recieve(ser):
    ...
    while ser.inWaiting() > 0:
        n = ser.inWaiting()
        out += ser.read(n)
    #print("read: H:", binascii.hexlify(out), "  ascii ", out)
    #print(out)    
    return out

# this sends and shows the sent message
def send(ser,m):
    #print("send: H: ",binascii.hexlify(m), "  ascii: ", m)
    #print(m)
    ser.write(m)
...
def convertResponseMessage(message):
    message1 = message[11]
    message2 = message[12]
    message3 = message[13]
    message4 = message[14]
    ...
    messageDEC = (message1*4096)
    messageDEC += (message2*256)
    messageDEC += (message3*16)
    messageDEC += message4
    return messageDEC

def convertLongResponseMessage(message):
    message1 = message[13]
    message2 = message[14]
    message3 = message[15]
    message4 = message[16]
    ...
    messageDEC = (message1*1048576)
    messageDEC += (message2*65536)
    messageDEC += (message3*4096)
    messageDEC += (message4*256)
    messageDEC += (message5*16)
    messageDEC += message6
    return messageDEC