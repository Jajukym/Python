...
# 1 complete cycle every 1 minute
timestep = 1  #seconds
routine = ((1,2),(31,58),(62,2),)
# routine2 added for resistance motor
routine2 = ((10,200),(35,30),(62,200),)
#
...
while cycling:
    ...
    if(time.time() > start+routine[i][0]):
        ...
        if(i>=len(routine)):
            i = 0
        # added for resistance motor
        retry = 0
        m2 = sb_lib.sendMsg(sbser,"61 5 %r" %potv )
        print(potv)
        r += 1 
        if(not len(m2)):
            retry = 1;
    ...
    #or resend if there was no response
        ...
        if(not len(m)):
            retry = 1;
        # m2 added for resistance motor changes
        retry = 0;
        m2 = sb_lib.sendMsg(sbser,"61 5 %r" %potv )
            retry = 1;
        #       
